...
{
	var baseurl='http://localhost:8080/collaboration_backend'
		...
		{		            	
			 ...
			 return $http.post(baseurl+'/deletejob/' + jobid)
		            .then( 
		            	function (response) {
		                return response.data;
		            },
		    ...
		    deleteJobapp: function(jobid)
			{		            	

				 return $http.post(baseurl+'/deletejobapp/' + jobid)
			            .then( 
			            	function (response) {
			                return response.data;
			            },
		...
		fetchAllJobapp: function()
		{
				return $http.get(baseurl + "/alljobapp").then(function (response) 
				{	
					return response.data;
				},
				function(errResponse)
				{
					console.error('Error while fetching Jobapps' + errResponse);
					deferred.reject(errResponse);
				}
			  );
		},
